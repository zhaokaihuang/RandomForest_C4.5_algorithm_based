...
public class Id3 {
	...
	public Map<String, Integer> getConfusionMatrix(List<String> resultList, List<List<String>> testDataSet) {
		List<String> labelList = new ArrayList<String>();
		double total = resultList.size();
		double err = 0;
		Map<String, Integer> common = new HashMap<String, Integer>();
		...
		result.put("fn", 0);
		for (List<String> testData : testDataSet) {
			if(!common.containsKey(testData.get(testData.size() - 1))){
				common.put(testData.get(testData.size() - 1), 1);
			} else{
				common.put(testData.get(testData.size() - 1), common.get(testData.get(testData.size() - 1))+1);
			}
			labelList.add(testData.get(testData.size() - 1));
		}
		...
		while (ite.hasNext()) {
			Map.Entry<String, Integer> entry = ite.next();
			if (value < entry.getValue()) {
				positive = entry.getKey();
				value = entry.getValue();
			}
		}
		
		for(int i = 0; i < resultList.size(); i++){
			if(labelList.get(i).equals(positive)){//positive
				if(resultList.get(i).equals(labelList.get(i))){ //tp
					result.put("tp", result.get("tp")+1);
				}else{//fp
					result.put("fn", result.get("fn")+1);
				}
			} else if(!labelList.get(i).equals(positive)){//negative
				if(resultList.get(i).equals(labelList.get(i))){ //tn
					result.put("tn", result.get("tn")+1);
				}else{//fn
					result.put("fp", result.get("fp")+1);
				}
			}
		}
		return result;
